/**
 *
 * The transition to phase 3 should happen at RCL6
 ...
 *
 * TODO this phase will have to manage links
 * TODO need to update request queues and transport creeps to properly manage different resources.  Creeps should only execute pickup requests for minerals if there is a sink request for the mineral.
 The idea should be to maintain separate queues for each resource.
 *
 **/
 
 
// var roleBuilder = require('creep.phase2.static_builder');
var roleMiner = require('creep.phase3.link_miner');
// var roleTransport = require('creep.phase2.transport');
var roleUpgrader = require('creep.phase3.link_upgrader');
...
var room_market_manager = require('room.market');
...
var start_phase = function(room) {
    ...
    source_keepers.forEach(function(sk){
        var unsafe_source = sk.pos.findClosestByRange(sources);
        sources = _.filter(sources, function(s){ return s.id != unsafe_source.id;});
    });
    sources.forEach(function(source) {
        has_miner[source.id] = false;
    });
    room.memory[constants.MINER_TRACKER] = has_miner;
    ...
    room.memory[constants.NUM_MINERS] = 0;
    room.memory[constants.NUM_STATIC_BUILDER] = 0;
    room.memory[constants.NUM_UPGRADER] = 0;
...
/**
 * Don't spawn a miner for minerals without extractors
...
var run_room = function(room) {
    ...
    if (Game.time % 10 == 0) {
        ...
        room_market_manager.run_room_terminal(room);
    }
...
/**
 * if a mineral doesn't have an extractor, you ignore the missing miner.
var can_help = function(room) {
    return room.memory[constants.NUM_MINERS] >= room.memory[constants.SAFE_SOURCES].length
